...
// =============================================================================
//
// TYPESCRIPT STUDY GUIDE (API / Node.js)
// ---------------------------------------
// This file shows TypeScript in a Node.js backend context.
// Key concepts demonstrated here:
//
// 1. INTERFACES
//    Define the "shape" of an object — what properties it has and their types.
//    Unlike classes, interfaces have ZERO runtime cost (they're erased at compile time).
//    Example:
//      interface User { name: string; age: number; }
//      const user: User = { name: "Bob", age: 25 };
//
// 2. Record<K, V>
//    A built-in TypeScript utility type for objects with string keys and typed values.
//    Record<string, any> = an object where keys are strings and values can be anything.
//    It's shorthand for: { [key: string]: any }
//
// 3. Map<K, V>
//    TypeScript's generic Map type — a Map where keys are type K and values are type V.
//    Example: Map<string, UserSession> = a Map with string keys and UserSession values.
//
// 4. UNION TYPES (string | number)
//    A value that can be one of multiple types.
//    PORT can be a string (from env var) or number (the fallback 8080).
//
// 5. @ts-ignore
//    Tells TypeScript to skip type checking for the NEXT line.
//    Use sparingly — it's a last resort when you can't make TS happy otherwise.
//
// 6. GENERIC RETURN TYPES: Promise<{ status: number; data: any }>
//    Generics let you parameterize types. Promise<T> means "a promise that
//    resolves to type T". You can nest complex types inside generics.
//
// =============================================================================

const http = require('http');
const https = require('https');
// @ts-ignore — Node.js 20+ has a global `crypto` variable. The `@types/node` package
// declares it, which conflicts with our `const crypto = require('crypto')`.
// @ts-ignore tells TypeScript to skip this error. The code works fine at runtime.
...
// ": any" because the Stripe SDK returns a complex object and we don't need
// to fully type it for this project. "any" lets us call any method without TS errors.
const stripe: any = process.env.STRIPE_SECRET_KEY ? new Stripe(process.env.STRIPE_SECRET_KEY) : null;

// UNION TYPE: process.env.PORT is string | undefined, fallback 8080 is number.
// So PORT can be either string or number.
...
const API_URL: string = process.env.API_URL || `http://localhost:${PORT}`;

// Blizzard OAuth token cache (for client credentials)
let bnetAccessToken: string = '';
let bnetTokenExpiry: number = 0;

// =============================================================================
// INTERFACES — Define the shape of data structures
// =============================================================================
// Interfaces describe what properties an object MUST have and their types.
// They're compile-time only — they disappear completely in the output JavaScript.
// Think of them as "contracts" that objects must satisfy.

// A user's OAuth session data from Battle.net
interface UserSession {
    bnetId: string;       // Battle.net account ID
    battletag: string;    // Display name like "Player#1234"
    accessToken: string;  // OAuth token for API calls
    expiry: number;       // Unix timestamp when the session expires
}

// A user's saved progress (attunements they've checked off, BiS items obtained)
interface UserProgressData {
    attunements: Record<string, any>;  // Record<K,V> = object with string keys, any values
    bis: Record<string, any>;
}

// A donation supporter entry
interface Supporter {
    name: string;
    amount: number;
    date: string;
...
// TYPED MAPS — Map<KeyType, ValueType>
// TypeScript knows that userSessions.get("abc") returns UserSession | undefined
const userSessions: Map<string, UserSession> = new Map();
const userProgress: Map<string, UserProgressData> = new Map();

// Record<string, any> is used for JSON data loaded from files.
// The data has string keys but complex nested values we don't fully type.
let classData: Record<string, any> = {};
let itemIds: Record<string, any> = {};
let raidsData: Record<string, any> = {};
let recipesData: Record<string, any> = {};
...
function jsonResponse(res: any, data: any, status: number = 200): void {
    res.writeHead(status, { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' });
    res.end(JSON.stringify(data));
}

function errorResponse(res: any, message: string, status: number = 404): void {
    jsonResponse(res, { error: message }, status);
}

// Blizzard OAuth2 token fetching
function getBnetAccessToken(region: string): Promise<string> {
...
function bnetApiRequest(region: string, path: string, namespace: string, token: string): Promise<any> {
    return new Promise((resolve, reject) => {
        const req = https.request({
...
async function handleBnetCharacter(req: any, res: any, url: URL): Promise<void> {
    ...
    const charPath = `/profile/wow/character/${realmSlug}/${charName}/equipment`;

    console.log(`Fetching: ${charPath}`);

    let lastError: any = null;
    for (const namespace of namespaces) {
        console.log(`Trying namespace: ${namespace}`);
        try {
            const result = await bnetApiRequest(region, charPath, namespace, token);
            ...
            if (result.status === 200 && result.data.equipped_items) {
                const gear = result.data.equipped_items.map((item: any) => ({
                    id: item.item?.id,
...
async function handleItemSearch(req: any, res: any, url: URL): Promise<void> {
    const name = url.searchParams.get('name');
    const region = url.searchParams.get('region') || 'us';
...
async function handleCreateCheckoutSession(req: any, res: any): Promise<void> {
    if (!stripe) {
...
async function handleStripeWebhook(req: any, res: any): Promise<void> {
    if (!stripe) {
        return errorResponse(res, 'Stripe not configured', 503);
    }
...
// Get supporters list
function handleGetSupporters(req: any, res: any): void {
...
function getUserFromSession(req: any): UserSession | null {
...
function handleAuthLogin(req: any, res: any): void {
    const clientId = process.env.BNET_CLIENT_ID;
...
async function handleAuthCallback(req: any, res: any, url: URL): Promise<void> {
    ...
    const tokenData: any = await new Promise((resolve, reject) => {
        ...
        const tokenReq = https.request({
            ...
            let data = '';
            tokenRes.on('data', (chunk: any) => data += chunk);
            tokenRes.on('end', () => {
                try {
    ...
    const userInfo: any = await new Promise((resolve, reject) => {
        const userReq = https.request({
            ...
            let data = '';
            userRes.on('data', (chunk: any) => data += chunk);
            userRes.on('end', () => {
                try {
                    resolve(JSON.parse(data));
                } catch (e) {
...
function handleAuthUser(req: any, res: any): void {
...
// Handle logout
function handleAuthLogout(req: any, res: any): void {
    const cookies = parseCookies(req);
...
function handleProgress(req: any, res: any, method: string): void {
    const session = getUserFromSession(req);
    if (!session) {
...
const server = http.createServer((req: any, res: any) => {
    ...
    if (route[0] === 'classes') {
        if (!route[1]) {
            ...
            for (const [name, data] of Object.entries(classData) as [string, any][]) {
    ...
    if (route[0] === 'recipes') {
        if (!route[1]) {
            const list: Record<string, any> = {};
            for (const [name, data] of Object.entries(recipesData) as [string, any][]) {
